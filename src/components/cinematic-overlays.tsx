...
export function CinematicOverlays() {
  ...
  return (
    <>
      ...
      {/* Decorative space scenery — desktop only (visual noise + GPU cost on phones) */}
      <div className="hidden md:block">
        {/* EVE streaking across the sky */}
        <div className="fixed inset-x-0 top-[18%] z-[5] pointer-events-none">
          <EveShip className="w-28 md:w-36 animate-eve-fly drop-shadow-[0_0_18px_rgba(56,189,248,0.8)]" />
        </div>
      ...
      {/* CRT scanline sweep — desktop only */}
      <div className="hidden md:block fixed inset-0 z-[6] pointer-events-none overflow-hidden opacity-40">
        <div className="absolute left-0 right-0 h-24 bg-gradient-to-b from-transparent via-[rgba(56,189,248,0.06)] to-transparent animate-scanline" />
      </div>
    </>
  );
}
